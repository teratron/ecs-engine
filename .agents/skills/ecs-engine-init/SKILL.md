...
This skill cannot be invoked via `/ecs-engine:init` until the environment is initialized — `.claude/skills/` does not exist yet. Run the setup script directly from the project root:

**Windows:**

```powershell
powershell -NoProfile -File .agents/skills/ecs-engine-init/scripts/setup_windows.ps1
```

**Unix:**

```bash
bash .agents/skills/ecs-engine-init/scripts/setup_unix.sh
```

After the script runs, `.claude/skills/` will be a junction pointing to `.agents/skills/`, and the skill becomes available as `/ecs-engine-init` for subsequent re-initialization.
