...
1. `SocketBackend` and `LoopbackBackend` first — enables all other transport development to be tested without real sockets.
2. Reliability layer (ACK bitfield, retransmission, duplicate detection) — the core algorithm, independent of socket backend.
3. Connection lifecycle (handshake, heartbeat, disconnect) — depends on reliability for handshake packets.
4. Packet batching and MTU discovery — optimization layer, can be iterated after basic send/recv works.
5. `NetworkPlugin` and ECS integration last — ties everything together via SubApp and event bus.

## 7. Drawbacks & Alternatives

**UDP-only for game traffic**: Forces reimplementation of reliability in user-space. The alternative — TCP with Nagle disabled — avoids this complexity but introduces head-of-line blocking that degrades real-time gameplay. The trade-off favors UDP for any game with latency-sensitive state updates.

**No congestion control in v1**: Acceptable for LAN and controlled server environments. Internet-facing games with many simultaneous connections risk saturating links. Mitigation: the `max_send_rate` cap provides a basic ceiling. Full congestion control (CUBIC/BBR) is planned for v2.

**Single NetworkTransport instance**: Simplifies resource management but prevents running multiple independent transport layers (e.g., game traffic + voice chat on separate sockets). Alternative: allow multiple named transports registered in ServiceRegistry. Deferred to avoid premature complexity.
...
| :--- | :--- | :--- |
| 0.1.0 | 2026-03-28 | Initial draft — UDP transport, channel types, reliability layer, connection lifecycle, MTU discovery, network simulation |
