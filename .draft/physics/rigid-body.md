...
- [physics-server.md](physics-server.md) — Server that consumes RigidBody and manages PhysicsBodyHandle
...
This replaces the pattern of setting unrealistically large masses. A player character (`dominance = 10`) will always push physics props (`dominance = 0`) without being noticeably deflected.
...
`GlobalTransform` is transitively required by `Transform`. No explicit declaration needed here.
...
This keeps `RigidBody` a pure input component and avoids a "velocity" ECS component that would go stale between physics steps.
...
It is not a separate component type — just a bundle. The same `PhysicsPlugin` handles 2D and 3D bodies in a single simulation world.

### 4.11 Sleep Thresholds

Sleep sensitivity is configured globally in `PhysicsSettings` (physics-server §4.9):
...
Sleeping bodies are excluded from broad-phase, integration, and narrow-phase entirely. Large scenes with many physics props benefit significantly from low thresholds.
...
| :--- | :--- | :--- |
| 0.1.0 | 2026-03-27 | Initial draft — component fields, body types, mass properties, axis locks, sleep, dominance, lifecycle hooks |
